...
.dwt-center-pane-layer{position:fixed;left:min(var(--dsh-sidebar-width,280px),100vw);right:var(--dsh-host-right-panel-width,0px);top:0;bottom:0;z-index:24;display:flex;pointer-events:none}
...
`
      ...
      function useFuzzerState(historyStore, historyMeta) {
        ...
        const loadResult = React.useCallback(async item => {
          ...
          } catch (cause) { setError(cause?.message || String(cause)) } finally { setLoadingFlow(false) }
        }, [])
        ...
        const execute = React.useCallback(async spec => {
          ...
          try {
            ...
            const parsedAssertions = JSON.parse(assertionsValue || '{}')
            const response = await api('fuzz', { method: 'POST', body: JSON.stringify({ request: { raw: rawValue }, payloads: parsedPayloads, assertions: parsedAssertions, maxCases: Number(maxCasesValue), concurrency: Number(concurrencyValue), timeoutMs: Number(timeoutValue), network: networkValue }) })
      ...
      function useMitmState() {
        ...
        return { status, flows, selected, error, setError, busy, config, routesText, setRoutesText, suffixesText, setSuffixesText, autoRulesText, setAutoRulesText, haeRulesText, setHaeRulesText, responseOverride, setResponseOverride, responseStatus, setResponseStatus, responseHeadersText, setResponseHeadersText, updateConfig, saveConfig, toggleProxy, clear, refresh, select, flowAction }
      }

      function MitmConfigSidebar({ state }) {
        const { status, busy, config, error, routesText, setRoutesText, suffixesText, setSuffixesText, autoRulesText, setAutoRulesText, haeRulesText, setHaeRulesText, updateConfig, saveConfig, toggleProxy } = state
        ...
        return h('section', { className: 'dwt-mitm-sidebar', 'aria-label': 'MITM 配置' },
          ...
          h('div', { className: 'dwt-mitm-config' },
            ...
            h('div', { className: 'dwt-row dwt-mitm-actions' }, h('button', { className: 'dwt-btn primary', disabled: busy, onClick: toggleProxy }, status?.proxy ? '停止代理' : '启动代理'), h('button', { className: 'dwt-btn', onClick: saveConfig }, '保存配置')),
