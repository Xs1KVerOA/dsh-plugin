(function registerDshResourceCenter(global) {
  ...
  loader.load({
    ...
    factory(require) {
      ...
      const serviceManager = require('dsh-resource-center-service-manager')
...
.drc-hint{margin:12px 4px 0;padding-top:10px;border-top:1px solid var(--dsw-alias-border-l1,#e5e7eb);color:var(--dsw-alias-label-tertiary,#9aa0a8);font-size:10.5px;line-height:1.55}
...
`
      ...
      function WorkspaceTree(props) {
        ...
        const saveRename = (kind, id, title) => {
          ...
          setRenaming(null)
          if (kind === 'workspace') {
            if (props.workspaces && typeof props.workspaces.rename === 'function') props.workspaces.rename(id, next).catch(() => {})
          } else {
            requestRename(id, next).catch(() => {})
          }
        }
        const sessionMenu = (session) => [
          ...
          { id: 'fork-session', label: '分叉会话', run: () => {
            if (props.sessions && typeof props.sessions.fork === 'function') {
              props.sessions.fork({ sessionId: session.id }).then(id => { if (id && props.sessions.open) props.sessions.open(id) }).catch(() => {})
            }
          } },
          { separator: true },
          { id: 'archive-session', label: '归档会话', run: () => {
            if (props.workspaces && typeof props.workspaces.archiveSession === 'function') props.workspaces.archiveSession(session.id).catch(() => {})
          } },
        ]
        const workspaceMenu = (workspace) => [
          ...
          { id: 'delete-workspace', label: '删除工作区', danger: true, run: () => {
            if (props.workspaces && typeof props.workspaces.delete === 'function') props.workspaces.delete(workspace.workspaceId).catch(() => {})
          } },
        ]
        ...
        const createWorkspace = () => {
          ...
          props.workspaces.pickDirectory().then(path => {
            if (path && typeof props.workspaces.create === 'function') props.workspaces.create({ path }).catch(() => {})
          }).catch(() => {})
        }
...
(function registerDshResourceCenterServiceManager(global) {
  const loader = global.__ModuleLoader__
...
@media(max-width:900px){.dsm-center-pane-layer{left:min(var(--dsh-sidebar-width,280px),100vw)}}
