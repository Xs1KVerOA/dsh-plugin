...
export function apply(ctx) {
  ...
  ctx.tools.register(defineTool({
    ...
    description: '记录一个入口点驱动的代码审计候选。保留 Entry、Source、Sink、Chain、Guards、证据位置、Active、影响和修复建议，供候选复核和报告汇总；第一阶段始终进入待复核。',
    parameters: {
      runId: { type: 'string' }, candidateId: { type: 'string', required: true }, domain: { type: 'string' }, status: { type: 'string' }, severity: { type: 'string' }, title: { type: 'string' }, entryId: { type: 'string', required: true }, entryType: { type: 'string' }, entry: { type: 'string', required: true }, auth: { type: 'string' }, active: { type: 'string' }, source: { type: 'array', required: true, items: { type: 'string' } }, sink: { type: 'array', required: true, items: { type: 'string' } }, chain: { type: 'array', items: { type: 'string' } }, guards: { type: 'array', items: { type: 'string' } }, evidence: { type: 'array', required: true, items: { type: 'string' } }, evidenceLocations: { type: 'array', required: true, items: { type: 'json' } }, impact: { type: 'string', required: true }, confidence: { type: 'string' }, queueItem: { type: 'string' }, description: { type: 'string' }, remediation: { type: 'string' }, cvss: { type: 'string' }, cvssVector: { type: 'string' },
    },
    output,
  ...
  ctx.tools.register(defineTool({
    ...
    description: '复核一个代码审计候选。状态只能是 needs-review、confirmed、false-positive 或 accepted-risk；复核需要补充可达性、权限、防护、生产代码和证据充分性判断。confirmed 还必须有完整自检和有效 CVSS 3.1 向量，否则会保留为待复核。',
    parameters: {
      runId: { type: 'string' }, candidateId: { type: 'string', required: true }, status: { type: 'string', required: true }, cvssVector: { type: 'string' }, cvss: { type: 'string' }, confidence: { type: 'string' }, reviewNotes: { type: 'string' }, evidenceLocations: { type: 'array', items: { type: 'json' } }, reachable: { type: 'string' }, authorization: { type: 'string' }, inputValidation: { type: 'string' }, productionCode: { type: 'string' }, sufficientEvidence: { type: 'string' },
    },
    output,
  ...
  ctx.inject(['systemPrompt'], scope => scope.systemPrompt.section({
    ...
    text: () => '你是企业级代码安全审计助手。只使用 DSH 自身的业务流程，按“建立运行 → 产品理解 → API/入口提取 → 入口逐项分析 → 候选记录 → 候选复核 → 结构化报告”的顺序工作，不引入 AST 或额外编排阶段。先用 dsh_code_audit_start 记录目标、语言、范围和授权说明，再用 dsh_code_audit_update_understanding 提交产品用途、核心能力、功能边界、运行假设和技术栈。提取 API 后逐个分析：同时读取路由/入口、Handler、参数来源、调用的 Service/Repository、权限校验函数、关键配置、相关 Model/数据库操作、同功能错误处理和中间件；先搜索入口与符号，再搜索调用者和被调用者，长文件按函数或行号分块，并把已读取文件和相关符号写入 API 清单上下文。第一阶段只能用 dsh_code_audit_add_candidate 提出候选，候选必须关联已有 API，并提供 Entry、Source、Sink、Impact、证据说明和至少一个文件/行号/代码位置；不得直接把候选写成 confirmed。第二阶段逐个使用 dsh_code_audit_review_candidate，回答可达性、认证授权、输入过滤/编码、是否生产代码、证据是否充分五项自检；任何一项未知、无法判断或缺少有效 CVSS:3.1 向量，都只能是 needs-review。最终状态只能是 confirmed、needs-review、false-positive、accepted-risk；报告只把 confirmed 纳入已确认漏洞，needs-review、false-positive、accepted-risk 和未覆盖入口分别展示，并提供 API 覆盖率。不得执行破坏性 payload 或未授权动态验证。',
  }))
}
