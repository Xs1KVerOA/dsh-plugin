...
test('keeps APIs with different handlers separate and requires exact candidate association', async () => {
  ...
  await runtime.auditAddApi({ runId: run.id, entryId, method: 'GET', path: '/users', handler: 'Users.List' }, exec)
  ...
  const candidateBase = { runId: run.id, candidateId: 'HANDLER-001', entryId, entry: 'GET /users', source: ['users.go:10'], sink: ['users.go:20'], evidence: ['users.go:10-20'], evidenceLocations: [{ file: 'users.go', lineStart: 10, lineEnd: 20 }], impact: '对象访问控制可能缺失' }
  await assert.rejects(() => runtime.auditAddCandidate(candidateBase, exec), /多个 handler/)
  const candidate = await runtime.auditAddCandidate({ ...candidateBase, handler: 'Admin.Users.List' }, exec)
