...
test('stores code-audit API inventory, candidates, and structured reports separately', async () => {
  ...
  const run = await runtime.auditStart({ targetPath: '/repo/pam', auditMode: 'deep', language: 'go', scope: 'HTTP and RPC entrypoints', graphStatus: 'ready' }, exec)
  assert.equal(run.graphRequired, true)
  await runtime.auditAddApi({ runId: run.id, entryId: 'http:POST:/api/v1/login', entryType: 'http', method: 'POST', path: '/api/v1/login', handler: 'internal/auth.go:42', auth: 'public', active: 'yes', riskTags: ['auth'], targetPaths: ['internal/web'] }, exec)
  await runtime.auditAddCandidate({ runId: run.id, candidateId: 'AUTH-001', domain: 'auth', severity: 'medium', status: 'candidate', entryId: 'http:POST:/api/v1/login', entryType: 'http', entry: 'POST /api/v1/login', active: 'yes', source: ['internal/auth.go:42 body.token'], sink: ['internal/auth.go:88 session lookup'], chain: ['internal/auth.go:42 -> internal/auth.go:88'], guards: ['No rate limit'], impact: '认证接口缺少限流', confidence: 'medium' }, exec)
...
test('keeps audit IDs distinct when sanitized labels collide', async () => {
  ...
  await runtime.auditAddCandidate({ runId: run.id, candidateId: 'A/B' }, exec)
  await runtime.auditAddCandidate({ runId: run.id, candidateId: 'A_B' }, exec)
...
test('scores CVSS 3.1 base vectors and orders findings by score', async () => {
  ...
  await runtime.auditAddCandidate({ runId: run.id, candidateId: 'LOWER', title: '低优先级问题', entry: 'GET /low', cvssVector: 'CVSS:3.1/AV:N/AC:L/PR:L/UI:N/S:U/C:H/I:H/A:H' }, exec)
  await runtime.auditAddCandidate({ runId: run.id, candidateId: 'CRITICAL', title: '高优先级问题', entry: 'POST /admin', cvssVector: 'CVSS:3.1/AV:N/AC:L/PR:N/UI:N/S:U/C:H/I:H/A:H' }, exec)
...
test('does not trust a caller-supplied CVSS score without a valid vector', async () => {
  const runtime = createRuntime()
  const exec = auditExec('cvss-input-session')
  const run = await runtime.auditStart({ targetPath: '/repo' }, exec)
  const report = await runtime.auditReport({ runId: run.id, findings: [{ id: 'F-1', title: '未完成复核', severity: 'medium', cvssScore: 10 }] }, exec)
  assert.equal(report.findings[0].cvssScore, null)
  assert.equal(report.counts.medium, 1)
})
