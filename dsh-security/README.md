...
- Code-audit preset: `dsh_code_audit_start`, `dsh_code_audit_update_understanding`, `dsh_code_audit_add_api`, `dsh_code_audit_add_candidate`, and `dsh_code_audit_report` store product understanding, the entrypoint/API inventory, scoped candidates, and a verifier-approved structured report.
...
Profile configuration supplies the startup value; UI saves apply to the current running instance. Disabling the allowlist does not bypass DNS rebinding checks or the one-shot approval for private/internal targets. The legacy `allowPrivateTargets` field is retained for compatibility only and no longer bypasses approval. Reload or restart DSH after changing the profile.
