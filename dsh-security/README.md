...
Pentest tools are mounted only by `pentest`, and code-audit tools only by `code-audit`; ordinary presets do not expose them. The code-audit preset also explicitly denies the globally registered Web Fuzzer/MITM tools. The client selects the matching views from the current session or its ancestor chain. Each session header shows the actual mode. Pentest keeps the private-address/DNS/allowlist controls; code audit records product understanding, API inventory, candidates, and a structured report. `dsh_code_audit_start` accepts an HTTPS GitHub repository URL and shallow-clones it directly into the session workspace, so source retrieval does not go through penetration-testing tools or an external audit workflow. A legacy `security` preset remains as a compatibility entry point.
