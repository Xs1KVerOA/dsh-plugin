...
function normalizeConfig(config = {}) {
  return {
    listenHost: String(config.listenHost || '127.0.0.1'),
...
function networkHasCustomTransport(network) {
  return Boolean(
    network.proxy || network.ca || network.cert || network.key ||
    network.rejectUnauthorized === false || network.forceHttps || network.interceptHttps,
  )
}
...
async function connectThroughHttpProxy(target, input, network, timerSignal) {
  ...
  const proxySocket = await new Promise((resolve, reject) => {
    const request = proxyRequestFn({
      ...
      headers: {
        host: `${target.hostname}:${Number(target.port || 443)}`,
...
async function requestThroughSocks(target, input, network, timerSignal) {
  const proxy = network.proxy
  ...
  const { socket } = await SocksClient.createConnection({
    ...
    destination: { host: target.hostname, port: destinationPort },
  })
...
async function requestWithNodeTransport(target, input, network, timerSignal) {
  if (network.proxy?.protocol === 'socks5:' || network.proxy?.protocol === 'socks5h:') {
    return requestThroughSocks(target, input, network, timerSignal)
  }
  if (target.protocol === 'https:' && network.proxy) {
    return connectThroughHttpProxy(target, input, network, timerSignal)
  }
  if (network.proxy) {
    const proxy = network.proxy
...
async function requestWithFetch(state, input) {
  ...
  if (!state.config.allowPrivateTargets && isPrivateTarget(target)) throw new Error(`已阻止私有目标: ${target.hostname}`)
  ...
  if (!state.config.allowPrivateTargets && isPrivateTarget(ruledTarget)) throw new Error(`已阻止私有目标: ${ruledTarget.hostname}`)
  ...
  try {
    ...
    let response = networkHasCustomTransport(network)
      ? await requestWithNodeTransport(ruledTarget, input, network, timer.signal)
      : await (async () => {
          const fetched = await fetch(ruledTarget, {
            method: input.method,
            headers: upstreamHeaders(input.headers),
            body: input.body && !['GET', 'HEAD'].includes(input.method) ? input.body : undefined,
            redirect: input.redirect || 'manual',
            signal: timer.signal,
          })
          const body = Buffer.from(await fetched.arrayBuffer())
          return { status: fetched.status, headers: Object.fromEntries(fetched.headers.entries()), body, text: body.toString('utf8') }
        })()
...
function connectTunnel(state, req, clientSocket, head) {
  let target
  ...
  if (!state.config.allowPrivateTargets && isPrivateTarget(target)) {
...
export function makeRuntime(config) {
  ...
  async function startProxy(options = {}) {
    ...
    const host = String(options.host || state.mitm.listenHost || config.listenHost)
  ...
  async function browserRoute(req, res, url) {
    ...
    const result = await requestWithFetch(state, {
      ...
      headers: req.headers, body, redirect: 'follow',
    })
...
export function applyWebTesting(ctx, rawConfig = {}) {
  ...
  if (tools) {
    ctx.tools.register(defineTool({
      name: 'dsh_web_fuzzer',
      description: 'Run an authorized HTTP Web Fuzzer request matrix. Use {{name}} in method, URL, headers, or body and provide payloads.name as an array. Results are bounded and include response status, duration, assertions, and captured flow ids.',
      parameters: {
        ...
        assertions: { type: 'object', additionalProperties: true, description: 'Optional status, contains, notContains, regex, maxDurationMs assertions.' },
