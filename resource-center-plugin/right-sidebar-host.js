...
function getSettingsApi() {
	if (settingsApi !== void 0) return settingsApi;
	try {
		settingsApi = defaultRequire("@deepseek-ai/dsh-settings");
	} catch {
		settingsApi = {
...
function getToolsApi() {
	if (toolsApi !== void 0) return toolsApi;
	try {
		toolsApi = defaultRequire("@deepseek-ai/dsh-tools");
	} catch {
		toolsApi = { defineTool: (value) => value };
	}
	return toolsApi;
}
const settingsNamespace = (...args) => getSettingsApi().settingsNamespace(...args);
const defineTool = (...args) => getToolsApi().defineTool(...args);
...
const SIDEBAR_PREFS_NS = "dsh-resource-center-right-sidebar";
//#endregion
...
* @module dsh-resource-center-right-sidebar/config
*/
...
function decodeHtmlUrl(pathname) {
	if (!pathname.startsWith("/dsh-resource-center/sidebar/html/")) return {
		ok: false,
		status: 404,
		message: "not an html route"
	};
	const htmlPrefix = "/dsh-resource-center/sidebar/html/";
	const rest = pathname.slice(htmlPrefix.length);
	if (rest === "") return {
		ok: false,
		status: 400,
...
* (/dsh-resource-center/sidebar/bundle/<name>.js). The official /plugins/<id>/client.js route
...
* re-activation). Same browser-trust fence as every other /dsh-resource-center/sidebar route;
...
/**
* Build the /dsh-resource-center/sidebar/bundle route handler. `fence` is the shared browser-
* trust check every /dsh-resource-center/sidebar route applies; `chunkDir` is the directory the
...
function createBundleRouteHandler(fence, chunkDir = LIB_DIR) {
	return async (req, res) => {
		...
		const name = /^\/dsh-resource-center\/sidebar\/bundle\/([a-z0-9-]+)\.js$/.exec(pathname)?.[1];
...
/** Register the /dsh-resource-center/sidebar/bundle route (disposed with the fiber). */
function registerBundleRoute(ctx, fence) {
	return ctx.webServer.register({
		kind: "prefix",
		path: "/dsh-resource-center/sidebar/bundle",
...
* repair details from `/dsh-resource-center/sidebar/api/terminal.deps` (a WS close reason is
...
function isPluginRoot(dir) {
	...
	try {
		return JSON.parse(readFileSync(file, "utf8")).name === "dsh-resource-center-right-sidebar";
	} catch {
		return false;
	}
}
...
function createJobOutputMirror(ctx) {
	...
	ctx.effect(() => dispose, "dsh-resource-center-right-sidebar: job-output event mirror");
...
//#region src/index.ts
/**
* dsh-resource-center-right-sidebar host half: the /dsh-resource-center/sidebar JSON API (explorer listing, file
* read/write, git), the /dsh-resource-center/sidebar/file media route (images), the /dsh-resource-center/sidebar/html
* preview route, the /dsh-resource-center/sidebar/bundle lazy-chunk route (client code splits),
...
const name = "dsh-resource-center-right-sidebar";
...
/** Content type served by /dsh-resource-center/sidebar/file (binary-safe fallback for unknowns). */
	...
	return {
		...
		"settings.update": async (payload) => {
			...
			try {
				...
				const ConflictError = getSettingsApi().SettingsConflictError;
				if (typeof ConflictError === "function" && error instanceof ConflictError) throw new SidebarError("settings-conflict", error.message, 409);
...
function apply(ctx, config) {
	// Cordis exposes injected services as context properties, while the
	// resource-center host is also exercised by small composition contexts that
	// only implement get(). Normalize both forms once so the migrated workbench
	// does not make the combined plugin brittle.
	const hostContext = Object.create(ctx);
	// Cordis contexts expose injected services through a proxy setter. Assigning
	// an inherited service name on Object.create(ctx) therefore attempts to
	// re-provide the service and fails with "cannot set property ... in multiple
	// fibers" during a real profile boot. Define own data properties instead.
	Object.defineProperties(hostContext, {
		webServer: { value: ctx.webServer ?? ctx.get?.("webServer"), writable: true, configurable: true },
		sessions: { value: ctx.sessions ?? ctx.get?.("sessions"), writable: true, configurable: true },
		tools: { value: ctx.tools ?? ctx.get?.("tools"), writable: true, configurable: true },
		webRuntime: { value: ctx.webRuntime ?? ctx.get?.("webRuntime") ?? { trustedHosts: [] }, writable: true, configurable: true },
	});
	if (hostContext.webServer && typeof hostContext.webServer.registerUpgrade !== "function") {
		Object.defineProperty(hostContext, "webServer", { value: {
			...hostContext.webServer,
			registerUpgrade() { return () => {}; }
		}, writable: true, configurable: true });
	}
	if (typeof hostContext.inject !== "function") Object.defineProperty(hostContext, "inject", { value: () => {}, writable: true, configurable: true });
	ctx = hostContext;
	ensureSpawnHelper();
	...
	if (nodePty === null) {
		...
		ctx.logger?.warn(`[dsh-resource-center-right-sidebar] node-pty (${DSH_NODE_PTY_RANGE}) failed to load: ${detail}`);
	}
	...
	ctx.effect(() => ctx.webServer.register({
		kind: "prefix",
		path: "/dsh-resource-center/sidebar/api",
		handler: async (req, res) => {
			...
			const pathname = new URL(req.url ?? "/", "http://dsh.internal").pathname;
			const apiPrefix = "/dsh-resource-center/sidebar/api/";
			const method = pathname.startsWith(apiPrefix) ? pathname.slice(apiPrefix.length) : void 0;
	...
	}), "dsh-resource-center-right-sidebar: /dsh-resource-center/sidebar/api routes");
	ctx.effect(() => registerBundleRoute(ctx, fence), "dsh-resource-center-right-sidebar: /dsh-resource-center/sidebar/bundle chunk route");
	ctx.effect(() => ctx.webServer.register({
		kind: "prefix",
		path: "/dsh-resource-center/sidebar/file",
		handler: async (req, res) => {
			if (!fence(req)) {
				res.writeHead(403);
				res.end("forbidden");
	...
	}), "dsh-resource-center-right-sidebar: /dsh-resource-center/sidebar/file media route");
	ctx.effect(() => ctx.webServer.register({
		kind: "prefix",
		path: "/dsh-resource-center/sidebar/html",
		handler: async (req, res) => {
			if (!fence(req)) {
				res.writeHead(403);
				res.end("forbidden");
	...
	}), "dsh-resource-center-right-sidebar: /dsh-resource-center/sidebar/html preview route");
	const wss = new WebSocketServer({ noServer: true });
	ctx.effect(() => ctx.webServer.registerUpgrade({
		path: "/dsh-resource-center/sidebar/ws/terminal",
	...
	}), "dsh-resource-center-right-sidebar: terminal WebSocket");
	const agentListWss = new WebSocketServer({ noServer: true });
	ctx.effect(() => ctx.webServer.registerUpgrade({
		path: "/dsh-resource-center/sidebar/ws/agent-terminals",
		handler: (req, socket, head) => {
			if (!fence(req)) {
				socket.destroy();
				return;
	...
	}), "dsh-resource-center-right-sidebar: agent-terminals push WebSocket");
	ctx.effect(() => () => {
		toolsDisposers?.();
		...
		wss.close();
		agentListWss.close();
	}, "dsh-resource-center-right-sidebar: teardown");
}
