...
test('single resource-center bundle registers workspace, service-management, Test, and usage Activities', async () => {
  ...
  assert.match(source, /top:var\(--dsh-resource-center-top/, 'resource center center panes must consume the measured host top inset')
  assert.match(source, /bottom:var\(--dsh-resource-center-bottom/, 'resource center center panes must consume the measured host bottom inset')
  ...
  assert.match(source, /dwt-switch-input/, 'Web Fuzzer configuration must expose an enabled toggle')
  assert.match(source, /启用请求包配置/, 'Web Fuzzer request configuration must expose its own slider')
  assert.match(source, /启用网络配置/, 'Web Fuzzer network configuration must expose its own slider')
  assert.match(source, /启用并发配置/, 'Web Fuzzer concurrency configuration must expose its own slider')
  assert.match(source, /requestConfigEnabled/, 'Web Fuzzer request configuration slider must be part of state and history')
  assert.match(source, /networkConfigEnabled/, 'Web Fuzzer network configuration slider must be part of state and history')
  assert.match(source, /concurrencyConfigEnabled/, 'Web Fuzzer concurrency configuration slider must be part of state and history')
  assert.match(source, /Web Fuzzer 当前已停用/, 'disabled Web Fuzzer executions must be blocked before a request is sent')
  assert.match(source, /enabled: executionEnabled !== false/, 'the host must receive the Web Fuzzer enabled state')
  ...
  assert.match(source, /function parseEditableHttp/, 'Web Fuzzer should parse editable raw HTTP packets')
  assert.match(source, /function transformHttpRequest/, 'Web Fuzzer should expose request transformation actions')
  assert.match(source, /function RequestTransformMenu/, 'Web Fuzzer should expose a request transformation menu')
  assert.match(source, /改变 HTTP 方法为 GET/, 'Web Fuzzer should convert a request to GET')
  assert.match(source, /改变 HTTP 方法为 POST/, 'Web Fuzzer should convert a request to POST')
  assert.match(source, /multipart\/form-data/, 'Web Fuzzer should support multipart file-upload requests')
  assert.match(source, /filename="payload\.txt"/, 'Web Fuzzer upload conversion should create a file part')
  assert.match(source, /text\/plain; charset=utf-8/, 'Web Fuzzer should convert request bodies to text')
  assert.match(source, /application\/json; charset=utf-8/, 'Web Fuzzer should convert request bodies to JSON')
  assert.match(source, /application\/xml; charset=utf-8/, 'Web Fuzzer should convert request bodies to XML')
  assert.match(source, /application\/x-www-form-urlencoded; charset=utf-8/, 'Web Fuzzer should convert request bodies to form parameters')
  assert.match(source, /removeEditableHeader\(packet, 'content-length'\)/, 'Request transforms should remove stale content-length framing')
  assert.match(source, /removeEditableHeader\(packet, 'transfer-encoding'\)/, 'Request transforms should remove stale transfer framing')
  ...
  assert.match(source, /F-Regex \/ S-Regex \/ Scope \/ Format \/ Sensitive/, 'MITM HaE settings should explain the supported rule fields')
