...
test('single resource-center bundle registers workspace, service-management, Test, Hunter, and usage Activities', async () => {
  ...
  assert.match(source, /LLM 辅助查询/, 'Hunter must offer a natural-language syntax assistant above the query editor')
  assert.match(source, /生成 Hunter 语法/, 'Hunter assistant must expose an explicit generate action')
  assert.match(source, /assistQuery/, 'Hunter assistant must use its dedicated server action')
  assert.match(source, /仅生成并回填下方语法，不会自动发起 Hunter 查询。/, 'Hunter assistant must not auto-run a generated query')
  assert.match(source, /Hunter 尚未连接。配置 ApiKey 后将自动验证账号与额度。/, 'an unconfigured Hunter workspace must show setup guidance instead of a generic request failure')
  assert.match(source, /dhunter-card-filter/, 'Hunter saved-assets filtering must preserve the header title at narrow widths')
  assert.match(source, /dhunter-field-picker/, 'Hunter field selection must use a structured visual picker instead of raw comma-separated editing')
  assert.match(source, /管理字段与原始响应/, 'Hunter field selection must keep detailed controls behind a compact disclosure')
  assert.match(source, /原始响应字段按需开启/, 'Hunter field selection must disclose that expensive raw fields are opt-in')
  assert.match(source, /选择此组/, 'Hunter field selection must support selecting a complete field category')
  ...
  assert.match(source, /state\.setFields\(HUNTER_FIELDS\.join\(','\)\)/, 'Hunter must provide a full-field preset')
  ...
  assert.match(source, /已用内存/, 'SSH overview must label memory usage explicitly')
  assert.match(source, /剩余空间/, 'SSH overview must show available root-disk capacity explicitly')
  assert.match(source, /availableBytes/, 'SSH overview must consume available memory and disk capacity from inspect data')
  ...
  assert.match(source, /uploadObject/, 'S3 service manager must expose file uploads')
  assert.match(source, /S3_UPLOAD_MAX_BYTES = 1024 \* 1024 \* 1024/, 'S3 uploads must expose a 1 GiB client-side size limit')
  assert.match(source, /function uploadS3File/, 'S3 uploads must use a dedicated binary upload helper')
  assert.match(source, /\/api\/dsh-service-manage\/upload/, 'S3 uploads must avoid Base64 JSON transport')
  assert.match(source, /function S3Workspace/, 'S3 connections must use a dedicated object-management workspace')
  assert.match(source, /dsm-s3-workspace/, 'S3 object management must use a dedicated user-facing layout')
  assert.match(source, /对象详情/, 'S3 object management must present object details instead of raw operation output')
  assert.match(source, /op: 'listObjects'/, 'S3 object management must load object listings through the existing service API')
  assert.match(source, /onContextMenu: event =>/, 'S3 object rows must expose a context menu')
  assert.match(source, /引用到会话/, 'S3 object context menu must offer adding an object reference to a conversation')
  assert.match(source, /function createS3ObjectInputSource/, 'S3 object references must register an @ input source')
  assert.match(source, /name: 'MinIO 文件'/, 'S3 object @ references must be clearly labeled')
  assert.match(source, /op: 's3Reference'/, 'S3 object references must serialize through the ownership-aware service API')
  assert.match(source, /workspaces\.connectWorkspace\(workspaceId\)/, 'context-menu references must create or reuse a session in the selected workspace')
  assert.match(source, /input\.insertReference\(/, 'context-menu references must insert a structured @ reference rather than plain draft text')
  assert.match(source, /下载文件/, 'S3 object context menu must offer downloads')
  assert.match(source, /预览内容/, 'S3 object context menu must offer text previews')
  assert.match(source, /S3_PREVIEW_MAX_BYTES = 512 \* 1024/, 'S3 previews must have a bounded client-side size limit')
  assert.doesNotMatch(source, /onDoubleClick: downloadSelected/, 'S3 object rows must not download on double click')
  assert.match(source, /确认删除对象/, 'S3 object deletion must require a user confirmation')
  assert.match(source, /服务请求超时/, 'service requests must report timeout separately from user cancellation')
  assert.match(source, /requestRef\.current\?\.abort\(\)/, 'service overview must cancel only its own stale request')
  ...
  assert.equal(JSON.stringify(ctx.sidebar.getActivities().map(activity => activity.id)), JSON.stringify(['workspace', 'dsh-service-manage', 'test', 'hunter', 'usage-stats']))
  ...
  assert.equal(typeof ctx.sidebar.getActivities()[4].component, 'function')
  assert.deepEqual(sources.map(source => source.name), ['会话', '服务连接', 'MinIO 文件', 'Web Fuzzer', 'MITM', 'Hunter 资产'])
