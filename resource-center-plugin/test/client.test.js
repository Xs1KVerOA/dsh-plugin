...
test('single resource-center bundle registers workspace, service-management, Test, and usage Activities', async () => {
  ...
  assert.match(source, /conversation\.composer\.dock/, 'current session cost should attach to the host conversation footer slot')
  assert.match(source, /FJxK0a_root/, 'current session cost should attach to the host conversation footer')
  assert.match(source, /function currentSessionCostRoots/, 'current session cost should resolve stable and legacy footer roots')
